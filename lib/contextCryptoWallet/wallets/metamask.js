...
const CANCELLED = 'cancelled'; // signal that form is cancelled
    ...
    (function (win) {
        class MetaMaskWindows extends Wallet {
            ...
            async switchToNetwork(networkId) {
                let response;
                const instance = this;
                const EVENT_SWITCH_NETWORK = `Please use your ${capFirstChar(instance.name)} Wallet to switch from Network`;
                try {
                    instance.emitter.emit('info', EVENT_SWITCH_NETWORK, true);
                    response = await libMetamask.switchToNetwork(networkId);
                    instance.emitter.emit('cleanup', EVENT_SWITCH_NETWORK);
                }
                catch(err) {
                    instance.emitter.emit('cleanup', EVENT_SWITCH_NETWORK);
                    instance.emitter.emit(CANCELLED, CANCELLED); // code for being cancelled
                    throw new Error((err.code === 4100) ? 'Cannot send transaction: please unlock your Wallet first' : err.message);
                }
                return response;
            }
