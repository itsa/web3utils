...
class LedgerWallet extends HardwareWallet {
    ...
    _ledgerEvents(message, level) {
        const instance = this;
        const type = instance.eventSeverities[level] || 'message';
        instance.emitter.emit(type, message);
    }
    ...
    async sendTx(rawTx, {web3, extraPercentageGas}) {
        ...
        let x;
        try {
            x = instance.ledger.sendTx(rawTx, instance.address, accountIndex, instance.accounts[0], {web3, extraPercentageGas});
        }
        catch (err) {
            console.debug(err);
        }
