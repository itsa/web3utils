...
class Ledger {
    ...
    emit(message = '', level = 0, cleanup, needsToSign) {
        const instance = this;
        if (cleanup) {
            if (!message) {
                instance._events = {};
        ...
        if (typeof instance._eventsFn === 'function') {
            if (message === SIGNED) {
                instance._eventsFn(SIGNED, -2); // signed successfully
            }
            else if (!cleanup) {
                instance._eventsFn(message, level, !!needsToSign);
            } else {
                if (message) {
        ...
        if (sender) {
            try {
                ...
                if (eth) {
                    ...
                    const gasPriceBN = await getGasPrice(web3, extraPercentageGas);
                    rawTx.gasPrice = web3.utils.toHex(gasPriceBN);
                    rawTx.nonce = await getNonce(sender, web3, true);
                    rawTx.chainId = await web3.eth.getChainId();
                    delete rawTx.from; // clear: not needed on Ledger (would throw an error)
                    delete rawTx.gas; // clear: not needed on Ledger (would throw an error)
                    ...
                    instance.emit(EVENT_SIGN_TRANSACTION, 1, false, true);

                    const resolution = await ledgerService.resolveTransaction(unsignedTx.substring(2), null, {});
                    ...
                    instance.emit(SIGNED); // code for being signed
                    ...
                    await delay(1000);
                    return instance.sendTx(rawTx, sender, accountIndex, firstAddress, {web3, extraPercentageGas});
                }
            }
            catch (err) {
                let message = err.message;
                if (message.endsWith('(0x6985)')) {
                    message = 'Ledger device: Aborted';
                } else if ((message === 'insufficient funds for gas * price + value') || (message === 'Returned error: insufficient funds for gas * price + value')) {
                    message = 'Transaction failed: increase the Gasprice or retry later';
                }
